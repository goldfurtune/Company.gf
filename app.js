...
// SAME Bin ID + API keys as admin so front submissions appear in admin.
...
async function fetchBin() {
  ...
  data.claimCodes = Array.isArray(data.claimCodes) ? data.claimCodes : [];
  data.claimRequests = Array.isArray(data.claimRequests) ? data.claimRequests : [];
  return data;
}

/** Save full record. Always pass the data object returned by fetchBin(). */
async function saveBin(data) {
  if (!data || typeof data !== "object") throw new Error("saveBin requires data object");
  ...
  if (!res.ok) {
    ...
    throw new Error("Failed to save data (" + res.status + ") " + t);
  }
  return true;
}
...
function gfIdleLogout() {
  ...
  try {
    sessionStorage.setItem("gf_idle_logout", "1");
  } catch {}
...
function gfResetIdleTimer() {
  ...
  try {
    sessionStorage.setItem("gf_idle_last_activity", String(gfLastActivity));
  } catch {}
...
function showToast(msg, duration = 2800) {
  ...
  if (!t) {
    ...
    t.id = "toast";
    t.style.cssText =
      "position:fixed;top:20px;left:50%;transform:translateX(-50%) translateY(-80px);background:#241c0f;border:1px solid #e8c547;color:#f0e6c8;padding:12px 20px;border-radius:10px;z-index:999;opacity:0;transition:all 0.3s;font-size:0.9rem;";
